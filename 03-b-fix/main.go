...
func findAllLines(pattern string, fs []string) sync.Map {
	...
	for _, f := range fs {
		wg.Add(1)
		// (1) Change to use regexp.Regexp#Copy to avoid lock contention.
		copy := re.Copy()
		go func(exp *regexp.Regexp, f string) {
			defer wg.Done()
			r, err := findLines(exp, f)
			if err != nil {
				log.Println(err)
			}
			sm.Store(f, r)
...
func findLines(r *regexp.Regexp, f string) ([]string, error) {
	file, err := os.Open(f)
	if err != nil {
		return nil, err
	}
